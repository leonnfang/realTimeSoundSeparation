...
static int threadFunctionReadFromRawFile(void *ptr) {
    ...
    while (1) {
        ...
        if (elementsInBuffer >= pData->ringBuffer.bufferSize / NUM_WRITES_PER_BUFFER) {
            ...
            PaUtil_GetRingBufferWriteRegions(&pData->ringBuffer, elementsInBuffer, ptr + 0, sizes + 0, ptr + 1,
                                             sizes + 1);
            ...
            if (!feof(pData->file)) {
                ...
                int i;
                for (i = 0; i < 2 && ptr[i] != NULL; ++i) {
                    itemsReadFromFile += (ring_buffer_size_t) fread(ptr[i], pData->ringBuffer.elementSizeBytes,
                                                                    sizes[i], pData->file);
                }
...
//    ResumeThread(pData->threadHandle);
    pthread_t temp;
    pthread_create(&temp, NULL, fn, (void *) pData);
...
static int recordCallback(const void *inputBuffer, void *outputBuffer,
    ...
    ring_buffer_size_t elementsToWrite = rbs_min(elementsWriteable,
